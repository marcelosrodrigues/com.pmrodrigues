...
$('[id="pessoa.documento"]').mask("999.999.999-99");

function setMascara(element) {

    if ($(element).val() == 'PJ') {
        $('[id="pessoa.documento"]').mask("99.999.999/9999-99");
        $("#datanascimento").hide();
    } else {
        $('[id="pessoa.documento"]').mask("999.999.999-99");
        $("#datanascimento").show();
...
$("#adicionar-dependente").click(function () {
    var nome = $('[id="dependente.nome"]').val();
    var dataNascimento = $('[id="dependente.dataNascimento"]').val();
    var quantidade = $("#dependentes > table > tbody > tr ").length;
    if (nome != "" && dataNascimento != null) {
        $("#dependentes > table > tbody").append("<tr>"
            + "<td>" + nome
            + "<input type=\"hidden\" name=\"beneficiarios[" + quantidade + "].id\" value=\"\" />"
            + "</td>"
            + "<td>" + dataNascimento + "</td>"
            + "<td>" + $("[id='dependente.grauparentesco']").val() + "</td>"
        ...
        $('[id*="dependente.nome"]').val("");
        $('[id*="dependente.dataNascimento"]').val("");
        $('[id*="dependente.grauparentesco"]').val("");
...
$('[id="adicionar-telefone-cliente"]').click(function () {
    ...
    if ($('[id*="residencial.ddd"]').val() != "" && $('[id*="residencial.numero"]').val() != null) {
        ...
        $("ul#telefones").append("<li class=\"list-group-item\">" + telefone
            + "<input type=\"hidden\" name=\"pessoa.residenciais[" + $("ul#telefones > li").length + "].id\" value=\"\" />"
            + "<input type=\"hidden\" name=\"pessoa.residenciais[" + $("ul#telefones > li").length + "].ddd\" value=\"" + $('[id*="residencial.ddd"]').val() + "\" />"
            + "<input type=\"hidden\" name=\"pessoa.residenciais[" + $("ul#telefones > li").length + "].numero\" value=\"" + $('[id*="residencial.numero"]').val() + "\" />"
            + "<button id=\"remover-telefone\" telefone=\"" + telefone + "\" onclick='javascript:removerLI(this);' class=\"btn btn-danger btn-circle\" style='float: right; margin-top: -5px;' type=\"button\">"
            + "<i class=\"fa fa-minus\"></i>"
            + "</button>"
            + "</li>");
...
function doDelete(message) {

    var deleted = confirm(message);
    return deleted;
}

$("#usaPrimeiraParcela").click(function () {
    $('[id="object.usaPrimeiraParcela"]').val($(this).is(":checked"));
});

$("#qualicorp").click(function () {
    $('[id="object.qualicorp"]').val($(this).is(":checked"));
});