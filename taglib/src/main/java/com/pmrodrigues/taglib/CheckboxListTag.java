    ...
    public void doTag() throws JspException, IOException {
        ...
        try {
            ...
            for (final Object item : value) {
                out.write(format("<label class=\"%s-inline\">", this.getType()));

                final Object value = PropertyUtils.getProperty(item, valueField);
                final Object label = PropertyUtils.getProperty(item, labelField);

                out.write(format("<input type=\"%s\" name=\"%s\" id=\"%s\" value=\"%s\" %s>%s",
                        this.getType(), this.id, this.id, value, this.checked != null && this.checked.contains(item) ? "checked=\"checked\"" : "", label));
                out.write("</label>");
            }
            out.write("</div>");
            out.flush();
